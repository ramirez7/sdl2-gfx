...
withSomeSurface :: (Ptr Surface -> IO a) -> IO a
withSomeSurface = withSurface 1 2 3 4 5 6 7 8

withSomeRenderer :: (Renderer -> IO a) -> IO a
withSomeRenderer f = withSomeSurface $ \surface ->
  bracket (createSoftwareRenderer surface) destroyRenderer f
